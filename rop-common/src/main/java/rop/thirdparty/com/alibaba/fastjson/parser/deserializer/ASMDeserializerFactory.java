...
import rop.thirdparty.com.alibaba.fastjson.parser.JSONLexerBase;
import rop.thirdparty.com.alibaba.fastjson.parser.JSONToken;
import rop.thirdparty.com.alibaba.fastjson.parser.ParseContext;
import rop.thirdparty.com.alibaba.fastjson.parser.ParserConfig;
import rop.thirdparty.com.alibaba.fastjson.parser.SymbolTable;
import rop.thirdparty.com.alibaba.fastjson.parser.DefaultJSONParser.ResolveTask;
import rop.thirdparty.com.alibaba.fastjson.parser.deserializer.ASMJavaBeanDeserializer.InnerJavaBeanDeserializer;
...
public class ASMDeserializerFactory implements Opcodes {
    ...
    void _deserialzeArrayMapping(ClassWriter cw, Context context) {
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitLdcInsn(rop.thirdparty.com.alibaba.fastjson.asm.Type.getType(getDesc(fieldClass)));
                ...
                if (itemClass == String.class) {
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    mw.visitLdcInsn(rop.thirdparty.com.alibaba.fastjson.asm.Type.getType(getDesc(itemClass)));
    ...
    void _deserialze(ClassWriter cw, Context context) {
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, getType(DefaultJSONParser.class), "getContext",
                           "()Lrop/thirdparty/com/alibaba/fastjson/parser/ParseContext;");
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class) {
                ...
                if (itemClass == String.class) {
                    mw.visitLdcInsn(rop.thirdparty.com.alibaba.fastjson.asm.Type.getType(getDesc(fieldClass))); // cast
    ...
    private void _deserialze_list_obj(Context context, MethodVisitor mw, Label reset_, FieldInfo fieldInfo,
        ...
        mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_list_item_deser__",
                          getDesc(ObjectDeserializer.class));
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(rop.thirdparty.com.alibaba.fastjson.asm.Type.getType(getDesc(itemType)));
    ...
    private void _deserObject(Context context, MethodVisitor mw, FieldInfo fieldInfo, Class<?> fieldClass) {
        ...
        if (fieldInfo.getFieldType() instanceof Class) {
            mw.visitLdcInsn(rop.thirdparty.com.alibaba.fastjson.asm.Type.getType(getDesc(fieldInfo.getFieldClass())));
        } else {
            mw.visitVarInsn(ALOAD, 0);
            mw.visitLdcInsn(fieldInfo.getName());
