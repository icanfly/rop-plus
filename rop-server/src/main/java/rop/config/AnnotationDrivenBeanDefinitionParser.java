...
import rop.impl.AnnotationServletServiceRouterFactoryBean;
import rop.impl.DefaultServiceAccessController;
import rop.security.DefaultInvokeTimesController;
import rop.security.DefaultSecurityManager;
import rop.security.FileBaseAppSecretManager;
import rop.session.DefaultSessionManager;

/**
 * <pre>
 *     为注解&lt;rop:annotation/&gt;提供解析功能
 * </pre>
 *
 * @author 陈雄华
 * @author luopeng
...
public class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	public static final String ROP_THREAD_PREFIX = "rop-";
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		//设置时间戳的容忍度，即时间戳前后差异值区间
		setTimestampTolerance(element,serviceRouterDef);

        //设置TaskExecutor
        setTaskExecutor(element, parserContext, source, serviceRouterDef);

		//设置eventExecutor
		setEventExecutor(element,parserContext,source,serviceRouterDef);
	...
	private void setEventExecutor(Element element, ParserContext parserContext, Object source, RootBeanDefinition serviceRouterDef) {

		if (logger.isDebugEnabled()) {
			logger.debug("Rop开始配置事件异步调用线程池。");
		}

		//这里使用外部线程池，这样可以灵活配置
		RuntimeBeanReference taskExecutorReference;
		if(element.hasAttribute("event-executor")){
			taskExecutorReference = new RuntimeBeanReference(element.getAttribute("event-executor"));
			serviceRouterDef.getPropertyValues().add("eventPoolExecutor", taskExecutorReference);
		}

		if (logger.isDebugEnabled()) {
			logger.debug("Rop配置事件异步调用线程池完成。");
		}

	}

	private void setTimestampTolerance(Element element, RootBeanDefinition serviceRouterDef) {
		String timestampTolerance = element.getAttribute("timestamp-tolerance-seconds");
		if (!StringUtils.hasText(timestampTolerance)) {
			timestampTolerance = "-1";//无限制
		}
		if (logger.isDebugEnabled()) {
			logger.debug("Rop配置服务时间戳容忍度：{} s",timestampTolerance);
		}
		serviceRouterDef.getPropertyValues().addPropertyValue("timestampTolerance", timestampTolerance);
	}

	private void setUploadFileSetting(Element element, RootBeanDefinition serviceRouterDef) {
        String uploadFileMaxSize = element.getAttribute("uploadfile-maxsize-KB");
        if (StringUtils.hasText(uploadFileMaxSize)) {
            if (logger.isDebugEnabled()) {
                logger.debug("Rop配置文件最大上传大小为{}KB",uploadFileMaxSize);
            }
    ...
    private void setTaskExecutor(Element element, ParserContext parserContext, Object source, RootBeanDefinition serviceRouterDef) {
        if (logger.isDebugEnabled()) {
            logger.debug("Rop开始配置异步调用线程池。");
        }

		//这里使用外部线程池，这样可以灵活配置
		RuntimeBeanReference taskExecutorReference;
		if(element.hasAttribute("task-executor")){
			taskExecutorReference = new RuntimeBeanReference(element.getAttribute("task-executor"));
			serviceRouterDef.getPropertyValues().add("threadPoolExecutor", taskExecutorReference);
		}

		if (logger.isDebugEnabled()) {
			logger.debug("Rop配置异步调用线程池完成。");
		}
...
//
//		//设置线程名称
//		taskExecutorDef.getPropertyValues().addPropertyValue("threadNamePrefix",ROP_THREAD_PREFIX);
//
